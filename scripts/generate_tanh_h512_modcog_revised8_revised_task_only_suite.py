#!/usr/bin/env python3
"""Generate the complete revised-paper H512 task-preservation suite.

The capped S-NP rescale probe remains a separate suite because its existing
full-eight-task artifact uses one pruning seed per cell.
"""
...
SUITE_ID = "task_preservation_tanh_h512_modcog_revised8_12k_seqbest_revised_task_only_p50_80"
...
METHODS = (
    ...
    ("obs_compensated", "obs_compensated", (0,)),
)
SNP_STRATEGIES = {
...
def make_suite() -> dict:
    defaults = {
        ...
        # Official CTRNNs forbid recurrent self-connections, so no active
        # matched-diagonal correction is part of the revised protocol.
        ...
        "obs_damping": 0.001,
        "obs_max_samples": 25000,
        "obs_compensation_mode": "diagonal",
        "obs_exact_block_threshold": 128,
    }
    runs = []
    for network_seed in NETWORK_SEEDS:
        for task_label, task, ng_t in TASKS:
            ...
            runs.append({
                "run_id": f"taskpres_revised_{task_label}_netseed{network_seed}_baseline",
                "strategy": "none",
                "amount": 0.0,
                "no_prune": True,
                "seed": network_seed,
            ...
            for label, strategy, pruning_seeds in METHODS:
                for amount in AMOUNTS:
                    for pruning_seed in pruning_seeds:
                        run = {
                            "run_id": (
                                f"taskpres_revised_{task_label}_netseed{network_seed}_{label}_"
                        ...
                        if strategy in {"noise_prune", *SNP_STRATEGIES}:
                            run["noise_rng_seed"] = pruning_seed
                        if strategy in SNP_STRATEGIES or strategy == "obs_compensated":
