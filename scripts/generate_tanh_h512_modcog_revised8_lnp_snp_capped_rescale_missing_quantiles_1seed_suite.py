#!/usr/bin/env python3
"""Generate one-seed L-NP/S-NP capped-rescale missing-quantile suite.

This exploratory suite fills the cap-percentile curve without changing any
frozen paper artifacts. Existing data already cover q50 for L-NP/S-NP and q90
for S-NP. This suite runs q10/q20/q30/q40/q60/q70/q80 for both methods and q90
for L-NP so the combined one-seed curve spans q10 through q90 for both methods.
"""
...
COMMON_MISSING_QUANTILES = (0.10, 0.20, 0.30, 0.40, 0.60, 0.70, 0.80)
LNP_EXTRA_QUANTILES = (0.90,)
...
def make_suite() -> dict:
    ...
    for network_seed in NETWORK_SEEDS:
        for task_label, task, ng_t in TASKS:
            ...
            runs.append(
                {
                    "run_id": f"cap_curve_missing_{task_label}_netseed{network_seed}_baseline",
                    "strategy": "none",
                    "amount": 0.0,
                    "no_prune": True,
                    "seed": network_seed,
            ...
            method_specs = (
                ("lnp_capped_rescale", "noise_prune_capped_rescale", COMMON_MISSING_QUANTILES + LNP_EXTRA_QUANTILES),
                ("snp_capped_rescale", "simulation_noise_prune_capped_rescale", COMMON_MISSING_QUANTILES),
            )
            for label, strategy, quantiles in method_specs:
                for quantile in quantiles:
                    qtag = quantile_tag(quantile)
                    for amount in AMOUNTS:
                        run = {
                            "run_id": (
                                f"cap_curve_missing_{task_label}_netseed{network_seed}_"
                        ...
                        if strategy == "simulation_noise_prune_capped_rescale":
                            run["score_batch_seed"] = 100_000
                            run["score_batches_path"] = score_batches_path(task_label)
                        runs.append(run)
...
def main() -> None:
    ...
    print("expected completed rows: 1464 = 24 baselines + 768 L-NP capped + 672 S-NP capped")
