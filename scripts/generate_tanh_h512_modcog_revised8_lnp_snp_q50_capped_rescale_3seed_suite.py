#!/usr/bin/env python3
"""Generate the matched three-seed L-NP/S-NP q50 capped-rescale suite."""
...
METHODS = (
    ...
    ("lnp_capped_q50", "noise_prune_capped_rescale"),
    ("snp_rescale", "simulation_noise_prune_rescale"),
    ("snp_capped_q50", "simulation_noise_prune_capped_rescale"),
)
SNP_STRATEGIES = {
...
def make_suite() -> dict:
    ...
    for network_seed in NETWORK_SEEDS:
        for task_label, task, ng_t in TASKS:
            ...
            runs.append({
                "run_id": f"q50_cap_match_{task_label}_netseed{network_seed}_baseline",
                "strategy": "none",
                "amount": 0.0,
                "no_prune": True,
                "seed": network_seed,
            ...
            for label, strategy in METHODS:
                for amount in AMOUNTS:
                    for pruning_seed in PRUNING_SEEDS:
                        run = {
                            "run_id": (
                                f"q50_cap_match_{task_label}_netseed{network_seed}_{label}_"
                        ...
                        if strategy in CAPPED_STRATEGIES:
                            run["rescale_cap_quantile"] = CAP_QUANTILE
                        runs.append(run)
